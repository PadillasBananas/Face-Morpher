...
class Affine:
    ...
    def transform(self, sourceImage, destinationImage):
        ...
        for i in white:
            ...
            destinationImage[i[0]][i[1]] = np.round(pixel)
#        temp1 = Image.fromarray(destinationImage, 'L')
#        temp1.show()
        #imageio.imsave('{}.jpg'.format(self.source[0][0]), temp1)
'''
...
'''
    def generateMorphVideo(self, targetFolderPath, sequenceLength, includeReversed):
        ...
        Bl = Blender(self.startImage, self. startPoints, self.endImage, self.endPoints)
        j1 = 0
        for j in np.linspace(0,1,num = sequenceLength,endpoint = True):
            final = Bl.getBlendedImage(j)
            temp = Image.fromarray(final, 'L')
            temp.save("{}/frame{:03d}.jpg".format(targetFolderPath, j1), "JPEG")
            j1+=1
        for file in os.listdir("targetFolderPath"):
            if file.startswith('frame'):
                Dict.append("/{}/".format(targetFolderPath)+file)
        writ = imageio.get_writer('morph.mp4', fps = 5)
        for j in Dict:
            writ.append_data(imageio.imread(j))
        writ.close()
...
class ColorAffine:
    ...
    def transform(self, sourceImage, destinationImage):
        ...
        for i in white:
            ...
            destinationImage[i[0]][i[1]][0] = pixel
            destinationImage[i[0]][i[1]][1] = pixel1
            destinationImage[i[0]][i[1]][2] = pixel2
    ...
    def getBlendedImage(self, alpha):
        ...
        for i in self.tri.simplices:
            ...
            A2 = np.float_([self.endPoints[i[0]],#[0], self.endPoints[i[0]][1]],
                            self.endPoints[i[1]],#[0], self.endPoints[i[1]][1]],
                            self.endPoints[i[2]]])#[0], self.endPoints[i[2]][1]]])
            A3 = np.float_(np.multiply(A1,(1-alpha)))
            A4 = np.float_(np.multiply(A2, (alpha)))
            targ = np.add(A3, A4)
        ...
        im4[np.arange(self.startImage.shape[0])] = (1-alpha)*np.round(im1[np.arange(self.startImage.shape[0])])+alpha*np.round(im2[np.arange(self.startImage.shape[0])])
        #im3[np.arange(800)][np.arange(600)] = (1-alpha)*im1[np.arange(800)][np.arange(600)]+alpha*im2[np.arange(800)][np.arange(600)]
...
if __name__ == "__main__":
    ...
    #B.generateMorphVideo('targetFolderPath', 10, False)
    
    i = 0
    while i < 1.01:
        final = B.getBlendedImage(i)
        i += .5
        temp = Image.fromarray(final, 'RGB')
        temp.show()
